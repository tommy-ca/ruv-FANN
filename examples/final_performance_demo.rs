...
use ruv_fann::*;
use ruv_fann::training::{TrainingData, TrainingError, Adam, AdamW, IncrementalBackprop};
use std::time::Instant;

#[cfg(feature = "parallel")]
use ruv_fann::simd::{CpuSimdOps, SimdConfig, SimdMatrixOps, ActivationFunction as SimdActivation};
...
fn main() {
    ...
    let adam_result = test_optimizer("Adam", &xor_data, |_| {
    ...
    if let (Some(sgd), Some(adam), Some(adamw)) = (sgd_result, adam_result, adamw_result) {
        ...
        println!("├─────────────┼─────────────┼─────────────┼─────────────────┤");
        println!("│ SGD         │ {:>8}    │ {:>8.2}    │ {}          │", 
                 sgd_epochs, sgd_time, if sgd_epochs < 500 {"✅ Converged"} else {"❌ Failed"});
        println!("│ Adam        │ {:>8}    │ {:>8.2}    │ ✅ Converged     │", adam_epochs, adam_time);
        println!("│ AdamW       │ {:>8}    │ {:>8.2}    │ ✅ Converged     │", adamw_epochs, adamw_time);
        println!("└─────────────┴─────────────┴─────────────┴─────────────────┘");
        ...
        if sgd_epochs < 500 {
            ...
            println!("• Adam convergence speedup: {:.1}x faster ({} vs {} epochs)", adam_speedup, adam_epochs, sgd_epochs);
            println!("• AdamW convergence speedup: {:.1}x faster ({} vs {} epochs)", adamw_speedup, adamw_epochs, sgd_epochs);
        } else {
            println!("\n🎯 Optimization Results:");
...
fn test_optimizer<F>(name: &str, data: &TrainingData<f32>, create_trainer: F) -> Option<(usize, f64)>
where
    F: Fn(&Network<f32>) -> Box<dyn TrainingAlgorithmTrait<f32>>,
{
        ...
        match trainer.train_epoch(&mut network, data) {
            Ok(error) => {
                if epoch % 50 == 0 || error < target_error {
                    let outputs = data.inputs.iter()
                        .map(|input| network.run(input)[0])
                        .collect::<Vec<_>>();
                    println!("  Epoch {:3}: Error={:.6} Outputs=[{:.2}, {:.2}, {:.2}, {:.2}]", 
                             epoch, error, outputs[0], outputs[1], outputs[2], outputs[3]);
                }
                
                if error < target_error {
                    let elapsed = start_time.elapsed().as_secs_f64();
                    println!("  ✅ {} converged in {} epochs ({:.2}s)", name, epoch, elapsed);
    ...
    let elapsed = start_time.elapsed().as_secs_f64();
    println!("  ⚠️  {} did not converge in {} epochs ({:.2}s)", name, max_epochs, elapsed);
...
fn demo_simd_acceleration() {
    ...
    for size in sizes {
        ...
        println!("  {}x{} matrix: {:.1}x speedup ({:.4}s -> {:.4}s)", 
