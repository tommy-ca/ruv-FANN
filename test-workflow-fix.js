...
import { DAAService } from './ruv-swarm/npm/src/daa-service.js';
...
class WorkflowFixTestSuite {
  ...
  async testValidObjectWorkflow() {
    ...
    try {
      ...
      const log = mockAgent.getExecutionLog();
      
      const success = result.complete && result.stepsCompleted === 2 && log.length === 2;
      logTest('Valid object workflow execution', success, 
        `Completed: ${result.stepsCompleted}/${result.totalSteps}, Agent calls: ${log.length}`);
      
      if (TEST_CONFIG.VERBOSE) {
        log(`Agent execution log: ${JSON.stringify(log)}`);
      }
  ...
  async testMixedWorkflow() {
    ...
    try {
      ...
      const success = result.stepsCompleted >= 2 && log.length >= 2;
      logTest('Mixed workflow execution', success, 
        `Completed: ${result.stepsCompleted}/${result.totalSteps}, Valid calls: ${log.length}`);
