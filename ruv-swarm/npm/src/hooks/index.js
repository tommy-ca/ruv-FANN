...
EOF
    ...
    extractBulletPoints(output) {
        if (!output) return '- No specific achievements captured';
    ...
    getModifiedFilesList() {
        ...
        if (fileMap.size === 0) return 'No files modified';
...
`;
    ...
    extractUrlPatterns(url) {
        ...
        try {
            ...
            if (urlObj.pathname.endsWith('.md')) patterns.push('content:markdown');
            if (urlObj.pathname.includes('docs')) patterns.push('content:documentation');
            if (urlObj.pathname.includes('api')) patterns.push('content:api');
            if (urlObj.pathname.includes('guide')) patterns.push('content:guide');
