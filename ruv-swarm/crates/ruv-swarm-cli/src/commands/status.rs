...
fn display_swarm_overview(status: &SwarmStatus, output: &OutputHandler) {
    ...
    output.print_status("Health", &format!("{:.1}%", health), 
        if health >= 80.0 { StatusLevel::Good } 
        else if health >= 60.0 { StatusLevel::Warning } 
        else { StatusLevel::Error }
    );
    
    output.key_value(&[
        ("Created".to_string(), status.created_at.with_timezone(&Local).format("%Y-%m-%d %H:%M:%S").to_string()),
    ]);
}
...
fn display_agent_summary(agents: &[Agent], output: &OutputHandler) {
    ...
    let ready = agents.iter().filter(|a| matches!(a.status, AgentStatus::Ready)).count();
    let busy = agents.iter().filter(|a| matches!(a.status, AgentStatus::Busy)).count();
    let idle = agents.iter().filter(|a| matches!(a.status, AgentStatus::Idle)).count();
    let error = agents.iter().filter(|a| matches!(a.status, AgentStatus::Error(_))).count();
    let offline = agents.iter().filter(|a| matches!(a.status, AgentStatus::Offline)).count();
    
    output.key_value(&[
        ("Total Agents".to_string(), total.to_string()),
        ("Ready".to_string(), format!("{} ({}%)", ready, ready * 100 / total.max(1))),
        ("Busy".to_string(), format!("{} ({}%)", busy, busy * 100 / total.max(1))),
        ("Idle".to_string(), format!("{} ({}%)", idle, idle * 100 / total.max(1))),
        ("Error".to_string(), format!("{} ({}%)", error, error * 100 / total.max(1))),
        ("Offline".to_string(), format!("{} ({}%)", offline, offline * 100 / total.max(1))),
    ]);
    ...
    if !type_counts.is_empty() {
        output.info("\nAgent Types:");
        let type_list: Vec<String> = type_counts.iter()
            .map(|(t, c)| format!("{}: {}", t, c))
            .collect();
...
fn display_task_summary(tasks: &[Task], output: &OutputHandler) {
    ...
    let failed = tasks.iter().filter(|t| matches!(t.status, TaskStatus::Failed(_))).count();
    let timeout = tasks.iter().filter(|t| matches!(t.status, TaskStatus::Timeout)).count();
    
    output.key_value(&[
        ("Total Tasks".to_string(), total.to_string()),
        ("Pending".to_string(), pending.to_string()),
        ("Running".to_string(), running.to_string()),
        ("Completed".to_string(), format!("{} ({}%)", completed, completed * 100 / total.max(1))),
        ("Failed".to_string(), format!("{} ({}%)", failed, failed * 100 / total.max(1))),
        ("Timeout".to_string(), timeout.to_string()),
    ]);
    
    if total > 0 {
        let success_rate = completed as f32 / (completed + failed + timeout).max(1) as f32 * 100.0;
        output.print_status("Success Rate", &format!("{:.1}%", success_rate),
            if success_rate >= 90.0 { StatusLevel::Good }
            else if success_rate >= 70.0 { StatusLevel::Warning }
            else { StatusLevel::Error }
        );
    }
}
...
fn display_detailed_agents(agents: &[Agent], tasks: &[Task], output: &OutputHandler) {
    ...
    let headers = vec!["Name", "Type", "Status", "Tasks", "Success", "Uptime", "Memory", "CPU"];
    ...
    for agent in agents {
        ...
        let success_rate = if agent.metrics.tasks_completed > 0 {
            ...
            format!("{:.0}%", agent.metrics.tasks_completed as f32 / total as f32 * 100.0)
        } else {
            "N/A".to_string()
        };
        ...
        rows.push(vec![
            ...
            status_str,
            format!("{}/{}", agent.metrics.tasks_completed, agent.metrics.tasks_completed + agent.metrics.tasks_failed),
            success_rate,
            uptime_str,
...
fn display_active_tasks(tasks: &[Task], output: &OutputHandler) {
    ...
    let headers = vec!["ID", "Strategy", "Priority", "Status", "Agents", "Progress", "Duration"];
    let mut rows = Vec::new();
    ...
    for task in active_tasks {
        ...
        let progress = if !task.subtasks.is_empty() {
            let completed = task.subtasks.iter()
...
fn display_performance_metrics(agents: &[Agent], tasks: &[Task], output: &OutputHandler) {
    ...
    let avg_task_duration: u64 = if !agents.is_empty() {
        agents.iter().map(|a| a.metrics.avg_task_duration_ms).sum::<u64>() / agents.len() as u64
    } else {
        0
    };
    
    output.key_value(&[
        ("Total Tasks Completed".to_string(), total_tasks_completed.to_string()),
        ("Total Tasks Failed".to_string(), total_tasks_failed.to_string()),
        ("Average Task Duration".to_string(), format!("{}ms", avg_task_duration)),
    ]);
    
    // Top performing agents
    let mut agent_scores: Vec<(&Agent, f32)> = agents.iter()
    ...
    if !agent_scores.is_empty() {
        output.info("\nTop Performing Agents:");
        let top_agents: Vec<String> = agent_scores.iter()
            .take(5)
...
fn display_resource_usage(agents: &[Agent], output: &OutputHandler) {
    ...
    let avg_cpu: f64 = if !agents.is_empty() {
        agents.iter().map(|a| a.metrics.cpu_usage_percent).sum::<f64>() / agents.len() as f64
    } else {
        0.0
    };
    
    output.key_value(&[
        ("Total Memory Usage".to_string(), format!("{:.1} MB", total_memory)),
        ("Average CPU Usage".to_string(), format!("{:.1}%", avg_cpu)),
    ]);
    
    // High resource consumers
    let high_memory: Vec<&Agent> = agents.iter()
        .filter(|a| a.metrics.memory_usage_mb > 100.0)
        .collect();
    
    let high_cpu: Vec<&Agent> = agents.iter()
        .filter(|a| a.metrics.cpu_usage_percent > 50.0)
        .collect();
    
    if !high_memory.is_empty() {
        output.warning("\nHigh Memory Usage:");
        let mem_list: Vec<String> = high_memory.iter()
    ...
    if !high_cpu.is_empty() {
        output.warning("\nHigh CPU Usage:");
        let cpu_list: Vec<String> = high_cpu.iter()
...
fn display_alerts(
    ...
    let error_agents: Vec<&Agent> = agents.iter()
    ...
    if total_tasks > 10 && failed_tasks as f32 / total_tasks as f32 * 100.0 > config.monitoring.alerts.failure_rate_threshold {
        alerts.push(format!("High task failure rate: {:.1}%", failed_tasks as f32 / total_tasks as f32 * 100.0));
    }
    
    // Check for resource alerts
    for agent in agents {
        if agent.metrics.cpu_usage_percent > config.monitoring.alerts.cpu_threshold as f64 {
            alerts.push(format!("Agent '{}' CPU usage exceeds threshold: {:.1}%", agent.name, agent.metrics.cpu_usage_percent));
        }
        
        if agent.metrics.memory_usage_mb > config.agent.memory_limit as f64 * (config.monitoring.alerts.memory_threshold as f64) / 100.0 {
            alerts.push(format!("Agent '{}' memory usage near limit: {:.1} MB", agent.name, agent.metrics.memory_usage_mb));
        }
    }
...
fn calculate_agent_score(metrics: &AgentMetrics) -> f32 {
    ...
    let resource_efficiency = 1.0 - (metrics.cpu_usage_percent as f32 / 100.0 + metrics.memory_usage_mb as f32 / 512.0) / 2.0;
