...
impl WasmNeuralForecast {
    ...
    pub fn get_available_models() -> JsValue {
        ...
        let model_info: Vec<_> = models.iter()
            .map(|m| {
                serde_json::json!({
                    "name": m.name,
    ...
    pub fn process_time_series(
        ...
        let transforms: Result<Vec<_>, _> = transformations.iter()
        ...
        Ok(JsValue::from_str(&serde_json::to_string(&serde_json::json!({
...
impl WasmEnsembleForecaster {
    ...
    #[wasm_bindgen(constructor)]
    pub fn new(strategy: String, model_names: Vec<String>) -> Result<WasmEnsembleForecaster, JsValue> {
...
impl WasmModelFactory {
    ...
    pub fn get_all_models() -> JsValue {
        ...
        let model_list: Vec<_> = models.iter()
