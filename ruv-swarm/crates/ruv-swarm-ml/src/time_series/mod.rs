...
impl TimeSeriesData {
    ...
    pub fn std_dev(&self) -> f32 {
        ...
        let variance = self.values.iter()
            .map(|x| (x - mean).powi(2))
            .sum::<f32>() / (self.values.len() - 1) as f32;
        variance.sqrt()
    }
}
...
impl TimeSeriesProcessor {
    ...
    pub fn add_scaler(&mut self, name: String, scaler_type: ScalerType) {
        let scaler = Scaler {
            ...
            parameters: match scaler_type {
                ...
                ScalerType::Standard => ScalerParameters::Standard { mean: 0.0, std: 1.0 },
                ScalerType::Robust => ScalerParameters::Robust { median: 0.0, iqr: 1.0 },
    ...
    fn apply_transformation(
        ...
        let transformed_data = match transform_type {
            ...
            TransformationType::ExponentialSmoothing => self.exponential_smoothing_data(data, 0.3)?,
    ...
    fn standardize_data(&self, mut data: TimeSeriesData) -> Result<TimeSeriesData, String> {
        ...
        let variance = data.values.iter().map(|x| (x - mean).powi(2)).sum::<f32>() / data.values.len() as f32;
    ...
    /// Difference the data
    fn difference_data(&self, mut data: TimeSeriesData, order: usize) -> Result<TimeSeriesData, String> {
        if data.values.len() <= order {
            return Err("Not enough data points for differencing".to_string());
        }
        
        for _ in 0..order {
            let differenced: Vec<f32> = data.values.windows(2)
                .map(|w| w[1] - w[0])
                .collect();
    ...
    fn moving_average_data(&self, mut data: TimeSeriesData, window: usize) -> Result<TimeSeriesData, String> {
        if window == 0 || window > data.values.len() {
            return Err("Invalid window size".to_string());
        }
        
        let smoothed: Vec<f32> = data.values.windows(window)
        ...
        data.timestamps = data.timestamps[timestamp_offset..timestamp_offset + smoothed_len].to_vec();
        
        Ok(data)
    }
    
    /// Apply exponential smoothing
    fn exponential_smoothing_data(&self, mut data: TimeSeriesData, alpha: f32) -> Result<TimeSeriesData, String> {
    ...
    fn box_cox_transform(&self, mut data: TimeSeriesData, lambda: f32) -> Result<TimeSeriesData, String> {
    ...
    pub fn detect_seasonality(&self, data: &TimeSeriesData) -> SeasonalityInfo {
        ...
        SeasonalityInfo {
            ...
            seasonal_strength: if seasonal_periods.is_empty() { 0.0 } else { 0.5 }, // Placeholder
...
impl FeatureEngineering {
    ...
    pub fn create_lag_features(data: &TimeSeriesData, lags: &[usize]) -> Vec<Vec<f32>> {
        ...
        for &lag in lags {
            let lag_feature: Vec<f32> = (lag..n)
                .map(|i| data.values[i - lag])
                .collect();
    ...
    pub fn create_rolling_features(data: &TimeSeriesData, windows: &[usize]) -> HashMap<String, Vec<f32>> {
        ...
        for &window in windows {
            ...
            // Rolling mean
            let rolling_mean: Vec<f32> = data.values.windows(window)
                .map(|w| w.iter().sum::<f32>() / window as f32)
                .collect();
            features.insert(format!("rolling_mean_{}", window), rolling_mean);
            
            // Rolling std
            let rolling_std: Vec<f32> = data.values.windows(window)
                .map(|w| {
                    let mean = w.iter().sum::<f32>() / window as f32;
                    let variance = w.iter().map(|x| (x - mean).powi(2)).sum::<f32>() / window as f32;
                    variance.sqrt()
                })
                .collect();
    ...
    pub fn create_datetime_features(timestamps: &[f64]) -> HashMap<String, Vec<f32>> {
        ...
        let hour_of_day: Vec<f32> = timestamps.iter()
            .map(|&ts| ((ts / 3600.0) % 24.0) as f32)
            .collect();
        features.insert("hour_of_day".to_string(), hour_of_day);
        
        let day_of_week: Vec<f32> = timestamps.iter()
