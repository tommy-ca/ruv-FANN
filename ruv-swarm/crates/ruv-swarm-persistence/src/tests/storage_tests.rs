...
async fn test_task_operations() {
    ...
    for i in 0..5 {
        let task = TaskModel {
            ...
            created_at: Utc::now().timestamp(),
            updated_at: Utc::now().timestamp(),
            completed_at: if i >= 3 { Some(Utc::now().timestamp()) } else { None },
        };
...
async fn test_message_storage() {
    ...
    for i in 0..5 {
        let message = MessageModel {
            id: Uuid::new_v4().to_string(),
            sender_id: if i % 2 == 0 { agent1.clone() } else { agent2.clone() },
            recipient_id: if i % 2 == 0 { agent2.clone() } else { agent1.clone() },
