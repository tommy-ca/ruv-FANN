...
impl WasmStorage {
    ...
    fn from_js_value<T: serde::de::DeserializeOwned>(value: JsValue) -> Result<T, StorageError> {
        let json = value.as_string()
            .ok_or_else(|| StorageError::Serialization(serde_json::Error::custom("Invalid JS value")))?;
...
impl Storage for WasmStorage {
    ...
    async fn store_agent(&self, agent: &AgentModel) -> Result<(), Self::Error> {
        let tx = self.db.transaction(&["agents"], TransactionMode::ReadWrite)
            .map_err(|e| StorageError::Transaction(format!("{:?}", e)))?;
        
        let store = tx.object_store("agents")
    ...
    async fn get_agent(&self, id: &str) -> Result<Option<AgentModel>, Self::Error> {
        let tx = self.db.transaction(&["agents"], TransactionMode::ReadOnly)
            .map_err(|e| StorageError::Transaction(format!("{:?}", e)))?;
    ...
    async fn list_agents_by_status(&self, status: &str) -> Result<Vec<AgentModel>, Self::Error> {
        ...
        let values = index.get_all(Some(&JsValue::from_str(status)))
            .await
            .map_err(|e| StorageError::Database(format!("{:?}", e)))?;
    ...
    async fn store_task(&self, task: &TaskModel) -> Result<(), Self::Error> {
        let tx = self.db.transaction(&["tasks"], TransactionMode::ReadWrite)
            .map_err(|e| StorageError::Transaction(format!("{:?}", e)))?;
        
        let store = tx.object_store("tasks")
            .map_err(|e| StorageError::Database(format!("{:?}", e)))?;
    ...
    async fn get_task(&self, id: &str) -> Result<Option<TaskModel>, Self::Error> {
        let tx = self.db.transaction(&["tasks"], TransactionMode::ReadOnly)
            .map_err(|e| StorageError::Transaction(format!("{:?}", e)))?;
    ...
    async fn get_pending_tasks(&self) -> Result<Vec<TaskModel>, Self::Error> {
        ...
        let values = index.get_all(Some(&JsValue::from_str("pending")))
            .await
            .map_err(|e| StorageError::Database(format!("{:?}", e)))?;
        
        let mut tasks: Vec<TaskModel> = values.into_iter()
    ...
    async fn get_tasks_by_agent(&self, agent_id: &str) -> Result<Vec<TaskModel>, Self::Error> {
        ...
        let index = store.index("assigned_to")
            .map_err(|e| StorageError::Database(format!("{:?}", e)))?;
        
        let values = index.get_all(Some(&JsValue::from_str(agent_id)))
            .await
            .map_err(|e| StorageError::Database(format!("{:?}", e)))?;
    ...
    async fn store_event(&self, event: &EventModel) -> Result<(), Self::Error> {
        let tx = self.db.transaction(&["events"], TransactionMode::ReadWrite)
            .map_err(|e| StorageError::Transaction(format!("{:?}", e)))?;
    ...
    async fn get_events_by_agent(&self, agent_id: &str, limit: usize) -> Result<Vec<EventModel>, Self::Error> {
        let tx = self.db.transaction(&["events"], TransactionMode::ReadOnly)
            .map_err(|e| StorageError::Transaction(format!("{:?}", e)))?;
        
        let store = tx.object_store("events")
            .map_err(|e| StorageError::Database(format!("{:?}", e)))?;
        
        let index = store.index("agent_id")
            .map_err(|e| StorageError::Database(format!("{:?}", e)))?;
        ...
        let mut events: Vec<EventModel> = values.into_iter()
            .filter_map(|v| Self::from_js_value(v).ok())
            .collect();
    ...
    async fn get_events_by_type(&self, event_type: &str, limit: usize) -> Result<Vec<EventModel>, Self::Error> {
        ...
        let index = store.index("event_type")
            .map_err(|e| StorageError::Database(format!("{:?}", e)))?;
        
        let values = index.get_all(Some(&JsValue::from_str(event_type)))
            .await
            .map_err(|e| StorageError::Database(format!("{:?}", e)))?;
    ...
    async fn get_events_since(&self, timestamp: i64) -> Result<Vec<EventModel>, Self::Error> {
        ...
        let events: Vec<EventModel> = values.into_iter()
            .filter_map(|v| Self::from_js_value(v).ok())
    ...
    async fn store_message(&self, message: &MessageModel) -> Result<(), Self::Error> {
        let tx = self.db.transaction(&["messages"], TransactionMode::ReadWrite)
            .map_err(|e| StorageError::Transaction(format!("{:?}", e)))?;
    ...
    async fn get_messages_between(&self, agent1: &str, agent2: &str, limit: usize) -> Result<Vec<MessageModel>, Self::Error> {
        ...
        let tx = self.db.transaction(&["messages"], TransactionMode::ReadOnly)
            .map_err(|e| StorageError::Transaction(format!("{:?}", e)))?;
        ...
        let mut messages: Vec<MessageModel> = values.into_iter()
            .filter_map(|v| Self::from_js_value(v).ok())
            .filter(|m| {
    ...
    async fn get_unread_messages(&self, agent_id: &str) -> Result<Vec<MessageModel>, Self::Error> {
        ...
        let index = store.index("to_agent")
            .map_err(|e| StorageError::Database(format!("{:?}", e)))?;
        ...
        let messages: Vec<MessageModel> = values.into_iter()
            .filter_map(|v| Self::from_js_value(v).ok())
            .filter(|m| !m.read)
            .collect();
    ...
    async fn mark_message_read(&self, message_id: &str) -> Result<(), Self::Error> {
        let mut message = match self.get_message(message_id).await? {
            Some(m) => m,
            None => return Err(StorageError::NotFound(format!("Message {} not found", message_id))),
        };
    ...
    async fn store_metric(&self, metric: &MetricModel) -> Result<(), Self::Error> {
        let tx = self.db.transaction(&["metrics"], TransactionMode::ReadWrite)
            .map_err(|e| StorageError::Transaction(format!("{:?}", e)))?;
        
        let store = tx.object_store("metrics")
            .map_err(|e| StorageError::Database(format!("{:?}", e)))?;
    ...
    async fn get_metrics_by_agent(&self, agent_id: &str, metric_type: &str) -> Result<Vec<MetricModel>, Self::Error> {
        let tx = self.db.transaction(&["metrics"], TransactionMode::ReadOnly)
            .map_err(|e| StorageError::Transaction(format!("{:?}", e)))?;
        ...
        let metrics: Vec<MetricModel> = values.into_iter()
            .filter_map(|v| Self::from_js_value(v).ok())
    ...
    async fn get_aggregated_metrics(&self, metric_type: &str, start_time: i64, end_time: i64) -> Result<Vec<MetricModel>, Self::Error> {
        ...
        let filtered_metrics: Vec<MetricModel> = values.into_iter()
            .filter_map(|v| Self::from_js_value(v).ok())
            .filter(|m| {
