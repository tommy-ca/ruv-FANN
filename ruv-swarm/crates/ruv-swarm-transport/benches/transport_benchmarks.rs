...
use ruv_swarm_transport::{
    ...
    shared_memory::{SharedMemoryTransport, SharedMemoryInfo},
    protocol::Message,
};
...
fn bench_in_process_broadcast(c: &mut Criterion) {
    ...
    for receivers in &[2, 5, 10, 20] {
        group.bench_with_input(BenchmarkId::from_parameter(receivers), receivers, |b, &receivers| {
            b.iter(|| {
                rt.block_on(async {
                    let config = TransportConfig::default();
                    let builder = ruv_swarm_transport::in_process::InProcessTransportBuilder::new();
                    
                    // Create sender
                    let sender = builder.build("sender".to_string(), config.clone()).await.unwrap();
                    
                    // Create receivers
                    let mut _receivers = Vec::new();
                    for i in 0..receivers {
                        let receiver = builder.build(format!("receiver{}", i), config.clone()).await.unwrap();
