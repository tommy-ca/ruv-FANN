...
impl TransportRegistry {
    ...
    pub async fn create(&self, name: &str, config: TransportConfig) -> Result<Box<dyn Transport<Message = Message, Error = TransportError>>, TransportError> {
        self.transports
            .get(name)
            .ok_or_else(|| TransportError::NotAvailable(format!("Transport '{}' not registered", name)))?
            .build(config)
            .await
    }
}
