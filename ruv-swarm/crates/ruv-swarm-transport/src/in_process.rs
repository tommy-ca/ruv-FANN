...
impl InProcessRegistry {
    ...
    async fn send(&self, from: &str, to: &str, msg: Message) -> Result<(), TransportError> {
        if let Some(endpoint) = self.transports.get(to) {
            endpoint.tx.send((from.to_string(), msg)).await
                .map_err(|_| TransportError::ConnectionError(format!("Transport {} is not receiving", to)))?;
            Ok(())
        } else {
            Err(TransportError::ConnectionError(format!("Transport {} not found", to)))
        }
    }
    ...
    async fn broadcast(&self, from: &str, msg: Message) -> Result<(), TransportError> {
        ...
        for entry in self.transports.iter() {
            ...
            if let Err(_) = endpoint.broadcast_tx.send(msg_clone) {
        ...
        if errors.is_empty() {
            ...
            Err(TransportError::Other(anyhow::anyhow!(
                "Failed to broadcast to: {}", errors.join(", ")
            )))
        }
    }
...
impl InProcessTransport {
    ...
    pub async fn new(id: String, config: TransportConfig, registry: Arc<InProcessRegistry>) -> Result<Self, TransportError> {
...
impl Transport for InProcessTransport {
    ...
    async fn send(&self, to: &str, msg: Self::Message) -> Result<(), Self::Error> {
        ...
        let size_estimate = bincode::serialized_size(&msg)
            .map_err(|e| TransportError::SerializationError(e.to_string()))? as usize;
    ...
    async fn receive(&mut self) -> Result<(String, Self::Message), Self::Error> {
        if !self.is_running.load(Ordering::SeqCst) {
            return Err(TransportError::ConnectionError("Transport is closed".to_string()));
        }
        
        let result = self.incoming_rx.recv().await
            .ok_or_else(|| TransportError::ConnectionError("Channel closed".to_string()))?;
        
        // Update stats
        let msg_size = bincode::serialized_size(&result.1)
            .unwrap_or(0) as u64;
...
impl InProcessTransportBuilder {
    ...
    /// Build a transport
    pub async fn build(&self, id: String, config: TransportConfig) -> Result<InProcessTransport, TransportError> {
...
mod tests {
    ...
    async fn test_in_process_transport_pair() {
        ...
        let msg = Message::event("agent1".to_string(), "test".to_string(), serde_json::json!({"data": "test"}));
    ...
    async fn test_in_process_broadcast() {
        ...
        let mut transport1 = builder.build("agent1".to_string(), config.clone()).await.unwrap();
        let mut transport2 = builder.build("agent2".to_string(), config.clone()).await.unwrap();
        let mut transport3 = builder.build("agent3".to_string(), config.clone()).await.unwrap();
    ...
    async fn test_message_size_limit() {
        ...
        let (transport1, _) = InProcessTransport::create_pair(
            "agent1".to_string(),
            "agent2".to_string(),
            config,
        ).await.unwrap();
    ...
    async fn test_registry_list() {
        ...
        // Create multiple transports
        let _t1 = InProcessTransport::new("agent1".to_string(), config.clone(), registry.clone()).await.unwrap();
        let _t2 = InProcessTransport::new("agent2".to_string(), config.clone(), registry.clone()).await.unwrap();
        let _t3 = InProcessTransport::new("agent3".to_string(), config, registry.clone()).await.unwrap();
