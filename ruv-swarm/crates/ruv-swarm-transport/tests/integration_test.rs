...
async fn test_in_process_communication() {
    ...
    let correlation_id = match &request.payload {
        ruv_swarm_transport::protocol::MessageType::Request { correlation_id, .. } => *correlation_id,
    ...
    match received_response.payload {
        ruv_swarm_transport::protocol::MessageType::Response { correlation_id: resp_id, result } => {
...
async fn test_broadcast() {
    ...
    // Create three agents
    let mut agent1 = builder.build("agent1".to_string(), config.clone()).await.unwrap();
    let mut agent2 = builder.build("agent2".to_string(), config.clone()).await.unwrap();
    let mut agent3 = builder.build("agent3".to_string(), config.clone()).await.unwrap();
