...
use crate::{ComparisonResult, ScenarioResult, StatisticalSignificance, ExecutionStatus};
...
impl PerformanceComparator {
    ...
    pub fn compare(
        ...
        let quality_improvement = self.calculate_quality_improvement(&baseline.metrics, &ml_optimized.metrics);
        let resource_efficiency = self.calculate_resource_efficiency(&baseline.metrics, &ml_optimized.metrics);
...
impl StatisticalAnalyzer {
    ...
    pub fn analyze_trials(
        ...
        let analysis = Analysis {
            ...
            confidence_interval_95: self.calculate_confidence_interval(&baseline_values, &ml_values, 0.95),
        };
    ...
    fn calculate_stats(&self, values: &[f64]) -> SummaryStatistics {
        ...
        SummaryStatistics {
            mean: values.mean(),
            median: if sorted_values.is_empty() { 0.0 } else { 
    ...
    fn calculate_median_improvement(&self, baseline: &[f64], optimized: &[f64]) -> f64 {
        ...
        let baseline_median = if baseline_sorted.is_empty() { 0.0 } else {
        ...
        let optimized_median = if optimized_sorted.is_empty() { 0.0 } else {
...
impl ComparisonFramework {
    ...
    pub fn execute_comparison(
        ...
        for (baseline, ml) in baseline_results.iter().zip(ml_results.iter()) {
            if baseline.scenario_name == ml.scenario_name {
                let comparison = self.comparator.compare(baseline, ml).unwrap_or_else(|_| {
                    ComparisonResult {
                        overall_improvement: None,
                        speed_improvement: None,
                        quality_improvement: None,
    ...
    fn calculate_aggregate_stats(
        ...
        AggregateStatistics {
            mean_overall_improvement: if improvements.is_empty() { 0.0 } else { improvements.clone().mean() },
            mean_speed_improvement: if speed_improvements.is_empty() { 0.0 } else { speed_improvements.mean() },
            scenarios_improved,
            scenarios_regressed,
...
impl ToString for Recommendation {
    fn to_string(&self) -> String {
        match self {
            Recommendation::StronglyRecommended => "Strongly Recommended - Significant improvements across most scenarios".to_string(),
            Recommendation::Recommended => "Recommended - Notable improvements with minimal regressions".to_string(),
            Recommendation::ConsiderWithCaution => "Consider with Caution - Mixed results, evaluate for specific use cases".to_string(),
            Recommendation::NotRecommended => "Not Recommended - No significant improvements or regressions observed".to_string(),
        }
    }
}
