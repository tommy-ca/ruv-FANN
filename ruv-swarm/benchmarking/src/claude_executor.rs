...
impl ClaudeCommandExecutor {
    ...
    pub async fn execute_benchmark(
        ...
        info!("Executing Claude command for scenario: {} ({})", scenario.name, mode.to_string());
        ...
        let parse_handle = tokio::spawn(async move {
            parser.process_stream(reader, &run_id_owned).await
        });
    ...
    pub async fn execute_swe_bench(
        ...
        let command = match mode {
            ExecutionMode::Baseline => {
                format!("solve SWE-bench instance {} without ML optimization", instance_id)
            }
            ExecutionMode::MLOptimized => {
                format!("solve SWE-bench instance {} using ML-optimized swarm coordination", instance_id)
            }
        };
        
        let args = vec![
            self.claude_path.to_string_lossy().to_string(),
            command,
            "-p".to_string(),
    ...
    fn build_command_args(&self, scenario: &BenchmarkScenario, mode: &ExecutionMode) -> Result<Vec<String>> {
        let mut args = vec![
            self.claude_path.to_string_lossy().to_string(),
        ];
        ...
        match mode {
            ...
            ExecutionMode::MLOptimized => {
                ...
                args.push(format!("{} with ML-optimized swarm coordination", scenario.claude_command));
            }
        }
    ...
    fn extract_swe_bench_results(&self, metrics: &PerformanceMetrics, instance_id: &str) -> SWEBenchResult {
        SWEBenchResult {
    ...
    pub async fn execute_batch(
        ...
        let results = stream::iter(scenarios)
            .map(|scenario| {
                ...
                let mode = mode.clone();
                let run_id = format!("{}-{}-{}", scenario.instance_id, mode.to_string(), uuid::Uuid::new_v4());
                
                async move {
                    executor.execute_benchmark(&scenario, mode, &run_id, None).await
                }
            })
...
mod tests {
    ...
    fn test_command_args_building() {
        ...
        let args = executor.build_command_args(&scenario, &ExecutionMode::Baseline).unwrap();
    ...
    fn test_batch_executor() {
        let batch = BatchExecutor::new(PathBuf::from("claude"))
