...
impl<T: Float + Send + Sync + Default + std::fmt::Debug + 'static> BatchGpuTrainer<T> {
    ...
    pub fn batch_forward_pass(
        ...
        for (layer_idx, layer) in network.layers.iter().skip(1).enumerate() {
            ...
            let (weight_matrix, biases) = self.extract_layer_parameters(layer, prev_activations[0].len(), current_layer_size);
            ...
            for mut output in batch_outputs {
                ...
                let activation_fn = layer.neurons.iter()
                    .find(|n| !n.is_bias)
    ...
    pub fn batch_compute_gradients(
        ...
        for layer_idx in (1..num_layers).rev() {
            ...
            let (layer_weights, _) = self.extract_layer_parameters(layer, prev_layer_size, current_layer_size);
            ...
            if layer_idx > 1 {
                ...
                for sample_idx in 0..batch_size {
                    ...
                    let activation_fn = prev_layer.neurons.iter()
                        .find(|n| !n.is_bias)
                        .map(|n| n.activation_function)
                        .unwrap_or(crate::ActivationFunction::Sigmoid);
                    
                    // Apply derivative based on activation function
                    for (i, &activation) in all_activations[layer_idx - 1][sample_idx].iter().enumerate() {
                        prev_errors[i] = prev_errors[i] * self.compute_activation_derivative(activation, activation_fn);
                    }
    ...
    fn compute_activation_derivative(&self, output: T, activation_fn: crate::ActivationFunction) -> T {
        use crate::ActivationFunction::*;
        
        match activation_fn {
            Sigmoid => output * (T::one() - output), // sigmoid'(x) = sigmoid(x) * (1 - sigmoid(x))
            Tanh => T::one() - output * output,      // tanh'(x) = 1 - tanh²(x)
            ReLU => if output > T::zero() { T::one() } else { T::zero() },
            Linear => T::one(),
...
pub fn batch_forward_with_activations<T: Float + Send + Sync + Default + std::fmt::Debug + 'static>(
    network: &Network<T>,
    batch_inputs: &[Vec<T>],
    backend: Arc<dyn ComputeBackend<T>>,
    ...
    for (layer_idx, layer) in network.layers.iter().skip(1).enumerate() {
        ...
        // Extract weights and biases once for the entire batch
        let (weight_matrix, biases) = batch_trainer.extract_layer_parameters(layer, prev_layer_size, current_layer_size);
...
pub fn gpu_batch_train_step<T: Float + Send + Sync + Default + std::fmt::Debug + 'static>(
    ...
    let batch_outputs: Vec<Vec<T>> = batch_activations.iter()
    ...
    let (weight_gradients, bias_gradients) = batch_trainer.batch_compute_gradients(
        network,
        &layer_activations,
        &batch_output_errors,
    )?;
    ...
    adam_params.apply_adam_updates_with_gradients(
        network,
        &weight_gradients,
        &bias_gradients,
    )?;
